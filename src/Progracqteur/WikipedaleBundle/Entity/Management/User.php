...
class User extends BaseUser
{
    ...
    public function isVirtualConsistant(\Symfony\Component\Validator\ExecutionContextInterface $context)
    {
        if ($this->isVirtual() === true) {
            if ($this->isLocked() === false) {
                $context->addViolationAt('locked', "admin.profile_user.inconsistent_virtual_lock", array(), null);
            }
        }
    }
    ...
    public function removeGroup(\FOS\UserBundle\Model\GroupInterface $group)
    {
        if ($this->getGroups()->contains($group)) {
            ...
            if (in_array($group->getType(), array(Group::TYPE_MANAGER or Group::TYPE_MODERATOR))) {
